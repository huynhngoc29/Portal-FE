    ...
    <div className="admin-layout">
      <aside className="admin-sidebar">
        ...
        <div style={{ marginTop: 'auto' }}>
           <button onClick={handleLogout} className="sidebar-link" style={{ width: '100%', background: 'transparent', border: 'none', textAlign: 'left', padding: '12px 16px' }}>
      ...
      <main className="admin-main">
        <header className="admin-header">
           <h2 style={{ fontSize: '1.2rem', fontWeight: 600 }}>Quản lý hệ thống</h2>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
             <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--accent-gradient)', display: 'grid', placeItems: 'center', fontWeight: 'bold' }}>
                {user.fullName.charAt(0)}
             </div>
             <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>{user.fullName}</span>
           </div>
        </header>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
          {user ? (
            <>
              {user.isAdmin && (
                <Link to="/admin" className="ghost-button" style={{ padding: "8px 16px", borderRadius: "10px" }}>
                  Admin Dashboard
                </Link>
              )}
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', padding: '0 8px' }}>
                {user.fullName}
              </span>
              <button type="button" className="ghost-button" onClick={handleLogout} style={{ padding: "8px 16px", borderRadius: "10px" }}>
                Đăng xuất
              </button>
            </>
          ) : (
             <>
               <Link to="/login" className="ghost-button" style={{ padding: "8px 16px", borderRadius: "10px" }}>
                 Đăng nhập
               </Link>
               <Link to="/register" className="primary-button" style={{ padding: "8px 16px", borderRadius: "10px" }}>
                 Tạo tài khoản
               </Link>
             </>
          )}
          ...
          <>
            ...
            <Route element={<AdminLayout user={authUser} />}>
              ...
              <Route path="/admin/*" element={
                <div className="state-box" style={{ marginTop: 40 }}>Tính năng đang phát triển</div>
              } />
            </Route>
